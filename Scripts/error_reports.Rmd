...
params:
  filename: "HFR_FY21_Jan_Cameroon_ CHAMP_20210215"
---
...
if(is_error("wrn_val_non-numeric-value")){
  
  issue_nonnumeric <- df_err_sbmsn %>%
    filter(validation_type == "wrn_val_non-numeric-value",
           !is.na(additional_information)) %>% 
    distinct(additional_information) %>% 
    pull() %>% 
    paste0(collapse = ", ")
  
  msg_issue_nonnumeric <- glue("**NON-NUMERIC WARNING**:
                       The submission contains {count_error('wrn_val_non-numeric-value')} of non-numeric result values in column L of your long format submission or column H onwards of a wide format submission. Review the submission to determine what is being entered so as to not continue with non-numeric values in future submissions.
                       \n{issue_nonnumeric}")
}
```

`r if(is_error("wrn_val_non-numeric-value")){msg_issue_nonnumeric}`
...
if(is_error("wrn_val_non-numeric-value ")){
  ...
  msg_nonnum <- glue('**NEGATIVE VALUE ERROR**:
                     Your file had {count_error("wrn_val_non-numeric-value ")} cell(s) with a non-numeric value reported. Review all value columns (column L in the long template or columns H onwards in the wide template) and update your submission, ensuring no cells have non-numeric values. All non-numeric values will otherwise be removed from the dataset. Non-numeric values were found in the following places:
                     \n{issue_val}')
...
`r if(is_error("wrn_val_non-numeric-value ")){msg_nonnum}`
